...
all_RES <- data.frame(beta0 = NULL, eta_0y = NULL, bias = NULL, se = NULL, method = NULL)
...
for (ii in 1:nrow(results_grid)) {
  ...
  results_grid$gmm_power[ii] <- mean(pchisq((beta0 + RES$rr_gmm_bias) ^ 2 / RES$rr_gmm_se ^ 2, df = 1, lower.tail = F) < 0.05)
  ...
  results_grid$gmm_bias[ii] <- mean(RES$rr_gmm_bias, na.rm = TRUE)
  results_grid$gmm_SE[ii] <- sd(RES$rr_gmm_bias, na.rm = TRUE)
  results_grid$gmm_mean_SE[ii] <- mean(RES$rr_gmm_se, na.rm = TRUE)
  results_grid$gmm_coverage[ii] <- mean(RES$rr_gmm_coverage, na.rm = TRUE)
  ...
  add_df <- data.frame(beta0 = param_grid$beta[ii],
                       ...
                       bias = with(RES, c(rr_gmm_bias, oracle_bias, logit_reg_bias, iptw_bias)),
                       se = with(RES, c(rr_gmm_se, oracle_se, logit_reg_se, iptw_se)),
                       method = rep(c("NC", "NC-Oracle", "Logit Reg.", "IPTW"), each = nrow(RES)))
  
  all_RES <- rbind(all_RES, add_df)
}

results_tab <- results_grid[, c("beta", "eta_0y", "ave_n",
...
results_tab_inf <- data.frame(beta0 = rep(round(results_tab$beta, 3), 4),
                              eta_0y = rep(round(results_tab$eta_0y, 3), 4),
...
## plot the bias
for (ee in log(0.01)) {
  plot_dat <- all_RES %>% filter(eta_0y == ee)
  ...
  ggsave(filename = paste0("plots/tnd_binary_bias_eta_0y_", round(ee * 100),"_.png"), plot = gg_bias,
         width = 6, height = 4)
  
  ## plot SE
  plot_dat_inf <- results_tab_inf %>% filter(eta_0y == round(ee, 3)) 
  ...
  gg_se <- ggplot(data = plot_dat, aes(x = method, y = se, color = method)) +
    ...
    theme(axis.title.x = element_blank(),
          ...
          legend.text = element_text(size = 14),
          legend.position = "bottom")
  ggsave(filename = paste0("plots/tnd_binary_se_eta_0y_", round(ee * 100),"_.png"), plot = gg_se,
         width = 6, height = 4)
  ...
  gg_coverage <- ggplot(data = plot_dat_inf, aes(x = method, y = coverage, fill = method)) +
    ...
    theme(axis.title.x = element_blank(),
          ...
          legend.text = element_text(size = 14),
          legend.position = "bottom")
  ggsave(filename = paste0("plots/tnd_binary_coverage_eta_0y_", round(ee * 100),"_.png"), plot = gg_coverage,
         width = 6, height = 4)
  ...
  gg_power <- ggplot(data = plot_dat_inf, aes(x = method, y = power, fill = method)) +
    ...
    theme(axis.title.x = element_blank(),
          ...
          legend.text = element_text(size = 14),
          legend.position = "bottom")
  ggsave(filename = paste0("plots/tnd_binary_power_eta_0y_", round(ee * 100),"_.png"), plot = gg_power,
         width = 6, height = 4)
}


for (bb in round(c(log(0.2), 0), 3)) {
  plot_dat <- results_tab_inf %>% filter(beta0 == bb) %>% mutate(delta = exp(eta_0y)) %>%
    filter(delta < 0.55)
  ...
  gg_bias_tr <- ggplot(data = plot_dat, aes(x = delta, y = bias, color = method)) +
    ...
    theme(axis.title.y = element_text(size = 18),
          ...
          legend.text = element_text(size = 14),
          legend.position = "bottom")
  ggsave(filename = paste0("plots/tnd_binary_bias_tr_beta0_", round(bb * 100),"_.png"), plot = gg_bias_tr,
         width = 6, height = 5)
  ...
  gg_coverage_tr <- ggplot(data = plot_dat, aes(x = delta, y = coverage, color = method)) +
    ...
    theme(axis.title.y = element_text(size = 18),
          ...
          legend.text = element_text(size = 14),
          legend.position = "bottom")
  ggsave(filename = paste0("plots/tnd_binary_coverage_tr_beta0_", round(bb * 100),"_.png"), 
         plot = gg_coverage_tr,
         width = 6, height = 5)
}
