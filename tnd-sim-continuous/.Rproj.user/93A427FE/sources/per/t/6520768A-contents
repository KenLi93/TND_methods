...
U_lr <- function(par, data){ ## dim = 4
  tau <- par
  U <- with(data, 
            c(A - expit(cbind(1, X) %*% tau)) * cbind(1, X)) 
  return(U)
}



U_rr <- function(par, data){ ## dim=1
  
  beta <- par[1]
  tau <- par[-1]
  pscore <- with(data, c(A / expit(cbind(1, X) %*% tau) + (1 - A) * (1 + exp(cbind(1, X) %*% tau))))
  U <- with(data,
            matrix((2 * A - 1) * Y * exp(-beta * A) * pscore, ncol = 1))
  return(U)
}

U_all <- function(par, data){ ## dim=1
  U <- cbind(U_lr(par[-1], data), U_rr(par, data))
  return(U)
}
...
tnd_iptw <- function(data) {
  ...
  ## first estimate q
  tau_est <- coef(glm(A ~ 1 + X, data = data, family = binomial))
  beta_est <- with(data,
                   log(mean(A * Y / expit(cbind(1, X) %*% tau_est)) / 
                         mean((1 - A) * Y * (1 + exp(cbind(1, X) %*% tau_est)))))
  
  ## use the estimated q to estimate the risk ratio
  param_est <- c(beta_est, tau_est)
  
  var_all <- var.gmmf(bfun = U_all, para = param_est, data)
